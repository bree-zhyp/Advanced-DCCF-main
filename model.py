...
class DCCF(nn.Module):
    def __init__(self, data_config, args):
        ...
        self.n_items = data_config['n_items']   # 项目数

        self.plain_adj = data_config['plain_adj']   # 交互矩阵的scipy.csr_matrix对象, 存在交互为1
        self.all_h_list = data_config['all_h_list']     # 头节点index: list
        self.all_t_list = data_config['all_t_list']     # 尾节点index: list
        self.A_in_shape = self.plain_adj.tocoo().shape  # (3 x 边数)
        self.A_indices = torch.tensor([self.all_h_list, self.all_t_list], dtype=torch.long).cuda()  # 交互稀疏矩阵的indices
        ...
        self.all_h_list = torch.LongTensor(self.all_h_list).cuda()  # 头节点tensor
        self.all_t_list = torch.LongTensor(self.all_t_list).cuda()  # 尾节点tensor
        self.G_indices, self.G_values = self._cal_sparse_adj()  # 归一化的邻接矩阵

        self.emb_dim = args.embed_size  # 嵌入特征维度
        self.n_layers = args.n_layers   # 层数
        self.n_intents = args.n_intents     # 意图嵌入维度
        self.temp = args.temp   # temperature in ssl loss

        self.batch_size = args.batch_size
        self.emb_reg = args.emb_reg     # 嵌入正则化系数
        self.cen_reg = args.cen_reg     # 意图正则化系数
        self.ssl_reg = args.ssl_reg     # 自监督正则化系数
        ...
        *********************************************************
        ...
        """
        self.user_embedding = nn.Embedding(self.n_users, self.emb_dim)  # 用户嵌入矩阵(n x emb_dim)
        self.item_embedding = nn.Embedding(self.n_items, self.emb_dim)  # 项目嵌入矩阵(n x emb_dim)

        # 构造并初始化用户意图嵌入矩阵(emb_dim x n_intents)
        ...
        # 构造并初始化项目意图嵌入矩阵(emb_dim x n_intents)
        ...
        *********************************************************
        Initialize Weights
        """
        self._init_weight()     # 用户嵌入和项目嵌入初始化

        self.alpha_gnn = torch.nn.Parameter(torch.tensor(1.0))
        self.alpha_int = torch.nn.Parameter(torch.tensor(1.0))
        self.alpha_gaa = torch.nn.Parameter(torch.tensor(1.0))
        self.alpha_iaa = torch.nn.Parameter(torch.tensor(1.0))

    def _init_weight(self):     # 仅在类初始化时被调用, 用户嵌入和项目嵌入初始化
        nn.init.xavier_normal_(self.user_embedding.weight)
        nn.init.xavier_normal_(self.item_embedding.weight)

    def _cal_sparse_adj(self):  # 仅在类初始化时被调用, 计算归一化的邻接矩阵    ->(indices, values)
    ...
    def _adaptive_mask(self, head_embeddings, tail_embeddings):
        ...
        D_scores_inv = A_tensor.sum(dim=1).pow(-1).nan_to_num(0, 0, 0).view(-1)     # 逆度评分
    ...
    def inference(self):
        ...
        for i in range(0, self.n_layers):
            ...
            # 首先计算用户嵌入向量与用户意图矩阵的点积并归一化，得到意图的权重分布, 最后将这些权重与意图矩阵进行点积，得到新的用户嵌入向量
            ...
            all_embeddings.append(self.alpha_gnn * gnn_layer_embeddings + self.alpha_int * int_layer_embeddings
                                  + self.alpha_gaa * gaa_layer_embeddings + self.alpha_iaa * iaa_layer_embeddings + all_embeddings[i])
